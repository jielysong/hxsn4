...
//    private UnbindDeviceTriggerHelper mTriggerHelper;
    ...
    private int probeDeviceInfo(String deviceSerial) {
        try {
            mEZProbeDeviceInfo = TApplication.getOpenSDK().probeDeviceInfo(serialNo);
...
//                            LocalInfo.getInstance().setWifiPassword(mac, wifiPassword);
        ...
        TApplication.getOpenSDK().stopConfigWiFi();
        TApplication.getOpenSDK().startConfigWifi(AutoWifiConnectingActivity.this, serialNo, wifiSSID, wifiPassword, mEZStartConfigWifiCallback);
    }
        ...
        new Thread() {
            public void run() {
                ...
                while (count > 0) {
                    ...
                    try {
                        TApplication.getOpenSDK().addDevice(serialNo, mVerifyCode);
...
//                        if (!TextUtils.isEmpty(mVerifyCode)) {
//                            //保存密码
//                            TApplication.getOpenSDK().setValidateCode(mVerifyCode, serialNo);
//                        }
