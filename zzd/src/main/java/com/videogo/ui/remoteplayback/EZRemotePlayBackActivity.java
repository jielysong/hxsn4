...
//                handleSearchEZDeviceFileSuccess();
    ...
    private void handleSearchEZCloudFileSuccess(Calendar seletedTime) {
        if (mEZMediaPlayer == null) {
            mEZMediaPlayer = TApplication.getOpenSDK().createPlayer(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo());
            if (mEZMediaPlayer == null)
                return;
...
//        mRemotePlayBackCaptureRl.setVisibility(View.VISIBLE);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    mCloudFileList = TApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(),starttime,endtime);
    ...
    private void searchEZCloudFileList(final Calendar seletedTime) {
        new Thread() {
            ...
            public void run() {
                try {
                    mEZCloudFileList = TApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), mStartTime, mEndTime);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    mEZDeviceFileList = TApplication.getOpenSDK().searchRecordFileFromDevice(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), startTime, endTime);
                    if (mEZDeviceFileList != null && mEZDeviceFileList.size() > 0) {
                    ...
                    mEZCloudFileList = TApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(),mEZAlarmInfo.getCameraNo(), startTime, endTime);
