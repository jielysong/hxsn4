                ...
                public void onClick(View v) {
                    ...
                    switch (v.getId()) {
                        ...
                        case R.id.ez_device_serial_layout:
                            try {
                                if (TApplication.getOpenSDK().getClass().isInstance(EZOpenSDK.class)) {
...
//            tv.setGravity(Gravity.CENTER);
    ...
    private class GetDeviceInfoTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
        	try {
        		mDeviceVersion = TApplication.getOpenSDK().getDeviceVersion(mEZDeviceInfo.getDeviceSerial());
        		return true;
        	} catch (BaseException e) {;
                ErrorInfo errorInfo = (ErrorInfo) e.getObject();
...
//            }
    ...
    private class SetDefenceTask extends AsyncTask<Boolean, Void, Boolean> {
		...
		protected Boolean doInBackground(Boolean... params) {
			...
			try {
				result = TApplication.getOpenSDK().setDefence(mEZDeviceInfo.getDeviceSerial(), bSetDefence? EZConstants.EZDefenceStatus.EZDefence_IPC_OPEN:
    ...
    private class OpenEncryptTask extends AsyncTask<Boolean, Void, Boolean> {
        ...
        protected Boolean doInBackground(Boolean... params) {
            ...
            try {
            	TApplication.getOpenSDK().setDeviceEncryptStatus(mEZDeviceInfo.getDeviceSerial(),mValidateCode,isEnableEncrypt);
...
//            	EZCameraInfo cameraInfo = params[0];
                TApplication.getOpenSDK().deleteDevice(mEZDeviceInfo.getDeviceSerial());
                return true;
            } catch (BaseException e) {
                ErrorInfo errorInfo = (ErrorInfo) e.getObject();
                mErrorCode = errorInfo.errorCode;
